...
local Mission = {
	...
	--Mission Variables--
	_Text1 = "OBJECTIVE: Escort your forces to the\n'landing area'.";
...
function RedSquadDeployment()
	...
	if ((Mission.AttackBool == true) and math.fmod(Mission.TurnCounter, SecondsToTurns(120)) == 0) then
		local AttackUnits = 1
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, GetPositionNear("spawnA", 0 , 10, 50)), "spawnaa");
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, GetPositionNear("spawnB", 0 , 10, 50)), "spawnbb");
			Goto(BuildObject(Mission.CPUTeamRace .. "varch",6, GetPositionNear("spawnC", 0 , 10, 50)), "spawncc");
			Goto(BuildObject(Mission.CPUTeamRace .. "vsent",6, GetPositionNear("spawnD", 0 , 10, 50)), "spawndd");
		end
	end
end
...
function TransportArrival()
	...
	if((Mission.AttackBool2 == true) and math.fmod(Mission.TurnCounter, SecondsToTurns(120)) == 0) then
		local AttackUnits = 1
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, GetPositionNear("spawnt1", 0 , 10, 50)), Mission.transport);
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, GetPositionNear("spawnt2", 0 , 10, 50)), Mission.transport);
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, GetPositionNear("spawnt3", 0 , 10, 50)), Mission.transport);
		end
	end
	...
	if ((Mission.ObjectiveSix == false) and (GetDistance(Mission.transport, "nav1") < 80.0)) then
   	...
   	if ((Mission.ObjectiveSeven == false) and GetDistance(Mission.transport, "dustoff") < 200.0) then
...
function TransportDeparture()
	if ((Mission.ObjectiveSeven == true) and (GetDistance(Mission.redLeader, Mission.DropShip) < 50.0)) then
...
function FailConditions()
	if ((Mission.TurnCounter > SecondsToTurns(900)) and (Mission.TurnCounter <= (SecondsToTurns(2220)))) then
		ai.checkMissionObjectStatus(ai, Mission.redTankLeader, "Captain Higgs is Dead", "failmessage.wav", Mission._Text12, "higgs.des", 15.0);
		ai.checkMissionObjectStatus(ai, Mission.redTurretLeader, "Lt. Miller is Dead", "failmessage.wav", Mission._Text13, "miller.des", 15.0);
	end

	if ((Mission.checkTrans == false )and (Mission.TurnCounter > (SecondsToTurns(2220)))) then
		ai.checkMissionObjectStatus(ai, Mission.transport, "Transport is Dead", "failmessage.wav", Mission._Text14, "transpo.des", 15.0);
	end

	if (Mission.TurnCounter > SecondsToTurns(5)) then -- Destroyed Enemy Recycler
		ai.checkMissionObjectStatus(ai, Mission.EnemyRecycler, "The player destroyed enemy recycler", "failmessage.wav", Mission._Text15, "", 5.0);
	end
end
...
function SurvivalLogic()
	...
	ai.spawnObjectsAtRepeatTimeWithOrder(Mission.TurnCounter, 120, 2, unitList, 1, 2); -- 2 minutes
end
