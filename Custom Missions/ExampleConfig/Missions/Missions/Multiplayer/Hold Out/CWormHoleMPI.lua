...
function Start()
	...
	print("File: mpinstant.lua optimized by General BlackDragon")
...
Ally(3, 15)
Ally(4, 15)
StartCockpitTimer(Mission.hour, 900,  300)
AddObjective("Objective: You and or your crew MUST hold out for 1 hour until Major Manson arrives! - ISDF Human Resources \n\nGreen Squad will periodically help your situation.", "green", 15.0);
...
local Zdarko=BuildObject("ivtank",2,"Zdarko_start");
SetObjectiveName(Zdarko, "Sgt. Zdarko");
SetObjectiveOn(Zdarko)
local minion1=BuildObject("ivscout",2,"Zdarko_escort1");
SetObjectiveName(minion1, "Cpt. Herndon");
Follow(minion1,Zdarko);
local minion2=BuildObject("ivscout",2,"Zdarko_escort2");
SetObjectiveName(minion2, "Pvt. J O'Neill");
Follow(minion2,Zdarko);		

local Masiker=BuildObject("ivtank",2,"Masiker_start");
SetObjectiveName(Masiker, "Sgt. Masiker");
SetObjectiveOn(Masiker)
local minion3=BuildObject("ivscout",2,"Masiker_escort1");
SetObjectiveName(minion3, "Cpt. Bramley");
Follow(minion3,Masiker);
local minion4=BuildObject("ivtank",2,"Masiker_escort2");
SetObjectiveName(minion4, "Lt. Durango");
Follow(minion4,Masiker);	
...
if(Mission.TurnCounter == SecondsToTurns(3600))then --3600 secs
...
local blah4;
if (Mission.TurnCounter == SecondsToTurns(3614)) then --- 3614 secs

	local AttackUnits = 1
		for i = 1, AttackUnits  do
			local foo = GetHandle("unnamed_ivdrop_land")
			RemoveObject(foo)
		print("3614 sec marker ");
		end
...
if (Mission.TurnCounter == SecondsToTurns(3613.5)) then --- 3613.5 secs
		...
		print("3613 sec marker ");
		DoGameover(18.0);
		end
		
if (Mission.TurnCounter == SecondsToTurns(3615)) then --- 3615 secs
...
if (math.fmod(Mission.TurnCounter, SecondsToTurns(120)) == 0) then --- 2 minutes
	...
	local AttackUnits = 3
		for i = 1, AttackUnits  do
			...
			Goto(BuildObject(Mission.CPUTeamRace .. "vatank",6, "spawnT"), "spawnT.1");	
		print("2 min marker ");
		end
end
...
if (Mission.TurnCounter == SecondsToTurns(180)) then --- 3 minutes
	local AttackUnits = 3
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, "spawn2"), "spawn2a");
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, "spawn2.1"), "spawn2b");
		print("3 min marker ");
		end
end
...
if (math.fmod(Mission.TurnCounter, SecondsToTurns(180)) == 0) then --- 3 minutes
	local AttackUnits = 5
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, "spawn3"), "spawn3a");
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, "spawn3.1"), "spawn3b");
...
if (Mission.TurnCounter == SecondsToTurns(600)) then --- 10 minutes
	local AttackUnits = 2
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, "spawn4"), "spawn4a");
			Goto(BuildObject(Mission.CPUTeamRace .. "vatank",6, "spawn4"), "spawn4a");
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, "spawn4.1"), "spawn4b");
		print("10 min marker ");
		end
end

------------------------------------------------------------------------
if (math.fmod(Mission.TurnCounter, SecondsToTurns(1740)) == 0) then --- 29 minutes
	AudioMessage("m_GoingAfterScavs.wav");
	local AttackUnits = 7
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, "spawn30"), "wave30");
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, "spawn30"), "wave30");
			Goto(BuildObject(Mission.CPUTeamRace .. "vserv",6, "spawnT"), "spawnT.1");
			Goto(BuildObject(Mission.CPUTeamRace .. "vatank",6, "spawnT1"), "spawnT.2");
		print("29 minute marker");
		end
end

------------------------------------------------------------------------
if (math.fmod(Mission.TurnCounter, SecondsToTurns(600)) == 0) then --- 10 minutes
	AudioMessage("isdf0602.wav");
	local AttackUnits = 10
		for i = 1, AttackUnits  do
			Goto(BuildObject(Mission.CPUTeamRace .. "vscout",6, "spawn15"), "wave15");
			Goto(BuildObject(Mission.CPUTeamRace .. "vtank",6, "spawn15"), "wave15");
			Goto(BuildObject(Mission.CPUTeamRace .. "vwalk",6, "spawnT1"), "spawnT.2");	
		print("10 Big minute marker");
		end
end
...
--Wave 4
-------
if (Mission.TurnCounter == SecondsToTurns(1500)) then --- 25 minute
local green1 = BuildObject("ivdrop_land",15,"landpoint");
			SetAngle(green1, 90)
			StartEmitter(green1, 1)
			StartEmitter(green1, 2)
...
if (Mission.TurnCounter == SecondsToTurns(1514)) then 
...
if (Mission.TurnCounter == SecondsToTurns(1513.25)) then 
			
	Goto(BuildObject("ivtank",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1513.50)) then 
			
	Goto(BuildObject("ivmbike",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1513.75)) then 
			
	Goto(BuildObject("ivmbike",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1530)) then 
green2 = GetHandle("unnamed_ivdrop")
SetObjectiveOff(green2)
SetAnimation(green2,"takeoff",1);
end

if (Mission.TurnCounter == SecondsToTurns(1540)) then 
green2 = GetHandle("unnamed_ivdrop")
RemoveObject(green2)
end
------------------------------------------------------------------------
...
--Wave 5
-------
if (Mission.TurnCounter == SecondsToTurns(1800)) then --- 30 minute
local green1 = BuildObject("ivdrop_land",15,"landpoint");
			SetAngle(green1, 90)
			StartEmitter(green1, 1)
			StartEmitter(green1, 2)
...
if (Mission.TurnCounter == SecondsToTurns(1814)) then 
...
if (Mission.TurnCounter == SecondsToTurns(1813.25)) then 
			
	Goto(BuildObject("ivtank",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1813.50)) then 
			
	Goto(BuildObject("ivmbike",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1813.75)) then 
			
	Goto(BuildObject("ivmbike",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(1830)) then 
green2 = GetHandle("unnamed_ivdrop")
SetObjectiveOff(green2)
SetAnimation(green2,"takeoff",1);
end

if (Mission.TurnCounter == SecondsToTurns(1840)) then 
green2 = GetHandle("unnamed_ivdrop")
RemoveObject(green2)
end
------------------------------------------------------------------------
...
--Wave 5
-------
if (Mission.TurnCounter == SecondsToTurns(2400)) then --- 30 minute
local green1 = BuildObject("ivdrop_land",15,"landpoint");
			SetAngle(green1, 90)
			StartEmitter(green1, 1)
			StartEmitter(green1, 2)
...
if (Mission.TurnCounter == SecondsToTurns(2414)) then 
...
if (Mission.TurnCounter == SecondsToTurns(2413.25)) then 
			
	Goto(BuildObject("ivtank",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(2413.50)) then 
			
	Goto(BuildObject("ivtank",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(2413.75)) then 
			
	Goto(BuildObject("ivtank",15, "greenpath"), "RecyclerEnemy");
	
end

if (Mission.TurnCounter == SecondsToTurns(2430)) then 
green2 = GetHandle("unnamed_ivdrop")
SetObjectiveOff(green2)
SetAnimation(green2,"takeoff",1);
end

if (Mission.TurnCounter == SecondsToTurns(2440)) then 
green2 = GetHandle("unnamed_ivdrop")
RemoveObject(green2)
end
------------------------------------------------------------------------
