...
function Start()
	...
	print("Special thanks to PredaHunter, FireRock, and Firefly for voice acting");
...
if ((Mission.ObjectiveOne == false) and (GetDistance(Mission.HumanRecycler,"nav1") < 100.0)) then
	...
	ClearObjectives();
	AddObjective(Mission._Text2, "green", 15.0);
	print("play audio 3");
...
if (Mission.TurnCounter == SecondsToTurns(900)) then --- 15 minutes
...
if ((Mission.ObjectiveTwo == false) and (GetDistance(Mission.PlayerH,"nav6") < 80.0)) then
...
if ((Mission.ObjectiveThree == false) and (GetDistance(Mission.redTurretLeader,"nav1") < 100.0)) then
	...
	ClearObjectives();
	AddObjective(Mission._Text5, "green", 15.0);
...
if(Mission.TurnCounter == (SecondsToTurns(960) + SecondsToTurns(600)))then
...
if(Mission.TurnCounter == (SecondsToTurns(2220)))then --2220
	print ("play audio 8");
	AudioMessage("audio8.wav");
	print("Player(s) is at phase 4 of Transport Mission");
	ClearObjectives();
	AddObjective(Mission._Text7, "green", 15.0);
...
--Follow(Mission.redLeader,Mission.transport);
		...
		Follow(Mission.minion2,Mission.redLeader);
		Follow(Mission.redLeader,Mission.transport);
...
if (not IsAlive(Mission.redTankLeader))then --Captain Higgs
	ClearObjectives();
...
if (not IsAlive(Mission.redTurretLeader))then --Lt. Miller
	ClearObjectives();
	AddObjective(Mission._Text11, "red", 15.0);
	DoGameover(15.0);
	Mission.notAroundBool = true;
...
if (not IsAlive(Mission.transport))then --Transport
	ClearObjectives();
	AddObjective(Mission._Text11, "red", 15.0);
	DoGameover(15.0);
	Mission.notAroundBool = true;
